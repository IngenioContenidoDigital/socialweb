...
switch ($action) {
	case 'send_request':
	$db->query("INSERT INTO friend_requests (user1,user2,time,accepted, encounter_date) VALUES ('".$user1."','".$user2."','".time()."','0',STR_TO_DATE('".$encounter."', '%Y-%m-%d'))");
	...
	echo '<a href="#" class="btn btn-default btn-block text-center mb-5" onclick="return false;">Cancel Booking</a>';
	break;
	case 'unfriend':
	$db->query("DELETE FROM friend_requests WHERE ((user1='".$user1."' AND user2='".$user2."') OR (user1='".$user2."' AND user2='".$user1."'))");
	echo '<p>Date: <input type="date" required id="datepicker" ></p><a href="#" class="btn btn-default btn-block text-center mb-5" onclick="return false;"><i class="ti-plus pull-right lh20"></i>'.$lang['Add_As_Friend'].'</a>';
	break;
	case 'accept_request':
